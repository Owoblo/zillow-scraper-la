...
export function getLocationBasedEmails(cityName) {
  ...
  // California cities - Bay Area (working cities only)
  const bayAreaCities = ['San Francisco', 'Santa Clara', 'Fremont', 'Daly City'];
  
  // California cities - Los Angeles Area (working cities only)
  const laAreaCities = ['Los Angeles', 'Glendale', 'Pasadena'];
  ...
  } else if (bayAreaCities.includes(cityName) || laAreaCities.includes(cityName)) {
