import { createClient } from '@supabase/supabase-js';
import { getAllCities } from './config/regions.js';
import DecodoZillowScraper from './decodo-scraper.js';
import { fetchSearchPage } from './zillow.js';
import dotenv from 'dotenv';

dotenv.config();

class RobustHybridZillowScraper {
  constructor() {
    this.supabase = createClient(
      process.env.SUPABASE_URL,
      process.env.SUPABASE_ANON_KEY
    );
    
    this.decodoScraper = new DecodoZillowScraper();
    this.metrics = {
      totalRequests: 0,
      successfulRequests: 0,
      failedRequests: 0,
      decodoRequests: 0,
      currentRequests: 0,
      totalListings: 0,
      rateLimitedCities: 0
    };
    
    this.batchSize = 5; // Process cities in smaller batches
    this.batchDelay = 30000; // 30 seconds between batches
    this.cityDelay = 5000; // 5 seconds between cities
  }

  /**
   * Robust scraping with rate limiting protection
   */
  async scrapeCitiesRobust(method = 'hybrid', cities = null) {
    console.log(`🚀 Starting Robust Hybrid Zillow Scraper (Method: ${method})`);
    console.log('================================================');
    
    const startTime = Date.now();
    
    try {
      const citiesToScrape = cities || this.getDefaultCities();
      console.log(`📍 Scraping ${citiesToScrape.length} cities in batches of ${this.batchSize}`);
      
      let allListings = [];
      const results = {
        success: true,
        method: method,
        cities: [],
        totalListings: 0,
        errors: [],
        metrics: this.metrics
      };
      
      // Process cities in batches to avoid rate limiting
      const batches = this.chunkArray(citiesToScrape, this.batchSize);
      
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        const batch = batches[batchIndex];
        console.log(`\n📦 Processing batch ${batchIndex + 1}/${batches.length}: ${batch.join(', ')}`);
        
        const batchResults = await this.processBatch(batch, method);
        allListings.push(...batchResults.listings);
        results.cities.push(...batchResults.cities);
        
        // Add delay between batches (except for the last one)
        if (batchIndex < batches.length - 1) {
          console.log(`⏳ Waiting ${this.batchDelay/1000}s before next batch...`);
          await this.delay(this.batchDelay);
        }
      }
      
      // Store results in smaller batches to avoid database timeout
      if (allListings.length > 0) {
        await this.storeListingsInBatches(allListings);
      }
      
      const duration = (Date.now() - startTime) / 1000;
      results.totalListings = allListings.length;
      results.duration = duration;
      results.metrics = this.metrics;
      
      console.log(`\n✅ Robust scraping completed in ${duration.toFixed(1)}s`);
      console.log(`📊 Total listings: ${allListings.length}`);
      console.log(`📊 Decodo requests: ${this.metrics.decodoRequests}`);
      console.log(`📊 Current requests: ${this.metrics.currentRequests}`);
      console.log(`📊 Rate limited cities: ${this.metrics.rateLimitedCities}`);
      console.log(`📊 Success rate: ${this.metrics.successfulRequests}/${this.metrics.totalRequests}`);
      
      return results;
      
    } catch (error) {
      console.error('❌ Robust scraping failed:', error.message);
      return {
        success: false,
        method: method,
        error: error.message,
        metrics: this.metrics
      };
    }
  }

  /**
   * Process a batch of cities
   */
  async processBatch(cities, method) {
    const batchListings = [];
    const batchCities = [];
    
    for (const cityName of cities) {
      console.log(`\n🏙️ Processing ${cityName}...`);
      
      try {
        let cityListings = [];
        
        if (method === 'current') {
          cityListings = await this.scrapeCityCurrent(cityName);
        } else if (method === 'decodo') {
          cityListings = await this.scrapeCityDecodo(cityName);
        } else if (method === 'hybrid') {
          // Try Decodo first, fallback to current if it fails
          try {
            cityListings = await this.scrapeCityDecodo(cityName);
            console.log(`✅ ${cityName}: ${cityListings.length} listings (Decodo)`);
          } catch (decodoError) {
            if (decodoError.message.includes('403') || decodoError.message.includes('Forbidden')) {
              console.log(`🚫 ${cityName}: Rate limited, skipping...`);
              this.metrics.rateLimitedCities++;
              batchCities.push({
                name: cityName,
                listings: 0,
                success: false,
                error: 'Rate limited',
                skipped: true
              });
              continue;
            } else {
              console.log(`⚠️ ${cityName}: Decodo failed, trying current system...`);
              cityListings = await this.scrapeCityCurrent(cityName);
              console.log(`✅ ${cityName}: ${cityListings.length} listings (Current)`);
            }
          }
        }
        
        batchListings.push(...cityListings);
        batchCities.push({
          name: cityName,
          listings: cityListings.length,
          success: true
        });
        
        // Add delay between cities
        await this.delay(this.cityDelay);
        
      } catch (error) {
        console.error(`❌ Error processing ${cityName}:`, error.message);
        batchCities.push({
          name: cityName,
          listings: 0,
          success: false,
          error: error.message
        });
      }
    }
    
    return {
      listings: batchListings,
      cities: batchCities
    };
  }

  /**
   * Scrape city using current system
   */
  async scrapeCityCurrent(cityName) {
    this.metrics.currentRequests++;
    this.metrics.totalRequests++;
    
    try {
      const cityConfig = this.getCityConfig(cityName);
      if (!cityConfig) {
        throw new Error(`City configuration not found for ${cityName}`);
      }
      
      const allListings = [];
      let page = 1;
      let hasMorePages = true;
      
      while (hasMorePages && page <= 20) {
        const pageListings = await fetchSearchPage(cityConfig, page);
        
        if (pageListings.length === 0) {
          hasMorePages = false;
          break;
        }
        
        allListings.push(...pageListings);
        page++;
        
        // Add delay between pages
        await this.delay(1000 + Math.random() * 2000);
      }
      
      this.metrics.successfulRequests++;
      this.metrics.totalListings += allListings.length;
      
      return allListings;
      
    } catch (error) {
      this.metrics.failedRequests++;
      throw error;
    }
  }

  /**
   * Scrape city using Decodo Advanced
   */
  async scrapeCityDecodo(cityName) {
    this.metrics.decodoRequests++;
    this.metrics.totalRequests++;
    
    try {
      const cityConfig = this.getCityConfig(cityName);
      if (!cityConfig) {
        throw new Error(`City configuration not found for ${cityName}`);
      }
      
      const allListings = await this.decodoScraper.scrapeCity(cityConfig);
      
      this.metrics.successfulRequests++;
      this.metrics.totalListings += allListings.length;
      
      return allListings;
      
    } catch (error) {
      this.metrics.failedRequests++;
      throw error;
    }
  }

  /**
   * Store listings in smaller batches to avoid database timeout
   */
  async storeListingsInBatches(listings, batchSize = 1000) {
    console.log(`💾 Storing ${listings.length} listings in batches of ${batchSize}...`);
    
    for (let i = 0; i < listings.length; i += batchSize) {
      const batch = listings.slice(i, i + batchSize);
      const batchNumber = Math.floor(i / batchSize) + 1;
      const totalBatches = Math.ceil(listings.length / batchSize);
      
      console.log(`📦 Storing batch ${batchNumber}/${totalBatches} (${batch.length} listings)...`);
      
      try {
        const { error } = await this.supabase
          .from('current_listings')
          .upsert(batch, { 
            onConflict: 'zpid',
            ignoreDuplicates: false 
          });
        
        if (error) {
          console.error(`❌ Error storing batch ${batchNumber}:`, error.message);
          throw error;
        }
        
        console.log(`✅ Batch ${batchNumber} stored successfully`);
        
        // Small delay between batches
        if (i + batchSize < listings.length) {
          await this.delay(1000);
        }
        
      } catch (error) {
        console.error(`❌ Failed to store batch ${batchNumber}:`, error.message);
        throw error;
      }
    }
    
    console.log(`✅ All ${listings.length} listings stored successfully`);
  }

  /**
   * Get city configuration
   */
  getCityConfig(cityName) {
    const allCities = getAllCities();
    return allCities.find(city => 
      city.name.toLowerCase() === cityName.toLowerCase()
    ) || null;
  }

  /**
   * Get default cities for testing
   */
  getDefaultCities() {
    return ['Toronto', 'Windsor']; // Start with just 2 cities for testing
  }

  /**
   * Split array into chunks
   */
  chunkArray(array, chunkSize) {
    const chunks = [];
    for (let i = 0; i < array.length; i += chunkSize) {
      chunks.push(array.slice(i, i + chunkSize));
    }
    return chunks;
  }

  /**
   * Delay utility
   */
  delay(ms) {
    return new Promise(resolve => setTimeout(resolve, ms));
  }
}

export default RobustHybridZillowScraper;
